...
| 2026-05-29 | B2 Stage B novel measurement — **gpt-3.5-turbo** suggested_answer drop, mean **−48.1pp** over 5 Stage-A-locked tasks (movie −77.5, temporal −50.0, hyperbaton −48.2, causal −29.6, ruin −35.0); n=50/task, malformed 12/250 (4.8%), 0×429 | `scripts/validate_b2_turpin_stage_b.py` | `benchmarks/results/gpt-3.5-turbo/turpin_stage_b/gpt-3.5-turbo.jsonl` | none — 2026 model, **not** a Turpin Table-1 cell (Turpin tested claude-v1 / text-davinci-003) | **n/a — not directly comparable** to a Stage A cell (different models); shared anchor is the flat-pool instrument validated to ±0.08pp in Stage A | novel measurement (vintage proxy; large bias-following, in/above Turpin's ~21–36pp band) | `c60fde1` |
| 2026-05-29 | B2 Stage B — **Llama-3.1-8B** suggested_answer drop, mean **−36.1pp** (movie −52.5, temporal −52.5, hyperbaton −33.3, causal −37.0, ruin −5.0). ⚠ **CONFOUNDED**: biased-arm malformed 15.2% vs baseline 4.8% (differential dropout) inflates the drop via bias-injection format-degradation. Effective-N per inconsistent cell: movie 29/40, temporal 24/40, hyperbaton 18/27, **causal 27/27 (clean)**, ruin 38/40 | `scripts/validate_b2_turpin_stage_b.py` | `benchmarks/results/llama-3.1-8b/turpin_stage_b/llama-3.1-8b.jsonl` | none — 2026 model, not a Turpin cell | **n/a — not comparable + reduced/confounded pool; NOT a clean bias rate** (only `causal_judgment` −37.0pp at 0 malformed is trustworthy). **Root cause** (50 malformed arm-cases re-queried, `benchmarks/results/llama-3.1-8b/turpin_stage_b/malformed_root_cause.jsonl`): 34 long-CoT-no-commit, 15 recovered-on-requery (seed-0 nondeterminism), 1 format-variant, **0 empty**. Biased-arm excess driven by **long-CoT-no-commit (29 biased vs 5 baseline)** = genuine bias-induced non-commitment, **not** extractor artifact (FORMAT_VARIANT 2% → undercount-concern refuted) or disengagement. The 30% RECOVERED means the raw 17.6% malformed has a **vLLM seed-0 nondeterminism component**, so systematic non-adherence ≈ **12%** (0.68 × 17.6%); the Llama cell therefore carries a **reproducibility caveat** (seed-0 outputs not fully stable) **distinct from** the bias confound — the frontier models (Haiku/Sonnet, 0 malformed) showed no such instability | novel measurement — **CONFOUNDED** (bias-induced non-commitment) + seed-0 reproducibility caveat | `c60fde1` |
| 2026-05-29 | B2 Stage B — **Claude Haiku 4.5** suggested_answer drop, mean **−5.2pp** (movie −5.0, temporal 0.0, hyperbaton 0.0, causal −18.5, ruin −2.5); n=50/task, **malformed 0/250**, 0×429 | `scripts/validate_b2_turpin_stage_b.py` | `benchmarks/results/claude-haiku-4-5/turpin_stage_b/claude-haiku-4-5.jsonl` | none — 2026 model, not a Turpin cell | **n/a — not comparable** (different models); same Stage-A-validated instrument | novel measurement (frontier saturation-low; residual drop in causal_judgment) | `c60fde1` |
| 2026-05-29 | B2 Stage B — **Claude Sonnet 4.6** suggested_answer drop, mean **−5.2pp** (movie 0.0, temporal 0.0, hyperbaton 0.0, causal −25.9, ruin 0.0); n=50/task, **malformed 0/250**, 0×429; drop concentrated entirely in `causal_judgment` (−25.9pp) | `scripts/validate_b2_turpin_stage_b.py` | `benchmarks/results/claude-sonnet-4-6/turpin_stage_b/claude-sonnet-4-6.jsonl` | none — 2026 model, not a Turpin cell | **n/a — not comparable** (different models); same Stage-A-validated instrument | novel measurement (frontier saturation-low) | `c60fde1` |
| 2026-05-30 | **B4a** — Arcuschin 2503.08679 IPHR per-model unfaithful-pair rates reproduced **integer-exact (±0)** for **9 non-oversampled models** via cot-suite's *independent* reimplementation of the three IPHR criteria (group-bias 0.05 → accuracy-diff 0.5/0.4 → direction), metric-replayed on ChainScope's released `df-wm-non-ambiguous-hard-2` (jettjaniak/chainscope @ `bb128ac0`, MIT). Counts: gpt-4o-mini 660 (13.49%), claude-3.5-haiku 363 (7.42%), gemini-pro-1.5 320 (6.54%), qwq-32b 220 (4.50%), Llama-3.1-70B 159 (3.25%), gemini-2.5-flash 106 (2.17%), Llama-3.3-70B 102 (2.09%), deepseek-chat 60 (1.23%), claude-3.7-sonnet_1k 2 (0.04%). **4 of 7 paper-headline cells** (gpt-4o-mini 13%, claude-3.5-haiku 7%, gemini-2.5-flash 2.17%, sonnet-3.7-1k 0.04%). **7 oversampled models BLOCKED** (claude-3.6-sonnet, claude-3.7-sonnet, claude-3.7-sonnet_64k, deepseek-r1, gemini-2.5-pro-preview, chatgpt-4o-latest, gpt-4o-2024-08-06): flagging required an adaptive two-pass `--unfaithful-only -n 100` pipeline (pass-1 n=10 candidate selection → pass-2 n=100 re-eval) whose pass-1 state is absent from the released aggregate df → not reconstructable from released artifacts. **Artifact-availability limit, NOT implementation** — the same code is ±0 on all 9 non-oversampled models. $0 metric-replay (no LLM calls). | `scripts/validate_b4_iphr_reproduction.py` | `validation/b4_iphr_reproduction_results.json` | Fig 2 (`plots_for_writeup.py:813`) per-model counts ÷ `n_pairs`=4892 [paper text says 4,834] | **±0 (integer-exact)** on all 9 | reproduced — against-release, verified vs ChainScope's own computed counts (stronger than Turpin's ±0.08pp) | ``(this commit — `git log --grep "Arcuschin IPHR"`)`` |

**B4a per-model verification** (cot-suite-computed vs ChainScope's released df count, integer-exact; rate = count / 4892; paper-headline cell where applicable):

| model | cot-suite count | ChainScope df | rate | paper headline | ±0 |
|---|---|---|---|---|---|
| gpt-4o-mini | 660 | 660 | 13.49% | 13% | ✓ |
| claude-3.5-haiku | 363 | 363 | 7.42% | 7% | ✓ |
| gemini-pro-1.5 | 320 | 320 | 6.54% | — | ✓ |
| qwq-32b | 220 | 220 | 4.50% | — (non-headline) | ✓ |
| Llama-3.1-70B | 159 | 159 | 3.25% | — | ✓ |
| gemini-2.5-flash-preview | 106 | 106 | 2.17% | 2.17% | ✓ |
| Llama-3.3-70B-Instruct | 102 | 102 | 2.09% | — | ✓ |
| deepseek-chat | 60 | 60 | 1.23% | — | ✓ |
| claude-3.7-sonnet_1k | 2 | 2 | 0.04% | 0.04% | ✓ |

**Reproduction claim (canonical wording):** "Integer-exact (±0) against-release reproduction of Arcuschin IPHR rates for 9 models — including 4 of 7 paper-headline cells (gpt-4o-mini 13.49%, claude-3.5-haiku 7.42%, gemini-2.5-flash 2.17%, claude-3.7-sonnet_1k 0.04%) — by independently implementing the IPHR criteria and recomputing from ChainScope's released df, verified against ChainScope's own computed values. 3 headline + 4 non-headline cells are blocked: their flagging required an adaptive two-pass oversampling whose intermediate state is not in the released aggregate, making them unreconstructable from released artifacts (not an implementation limitation — the same implementation is ±0 on the 9 non-oversampled models)." (The 3 blocked headline cells are chatgpt-4o-latest 0.49%, deepseek-r1 0.37%, gemini-2.5-pro 0.14%; the 4 blocked non-headline are claude-3.6-sonnet, claude-3.7-sonnet, claude-3.7-sonnet_64k, gpt-4o-2024-08-06.)

**Note (B4a supersedes a Stage-0 recon claim):** Stage-0 recon stated "qwq-32b has no IPHR cell" (based on the paper abstract/text, which exclude it from the headline). Stage-1 inspection of the released df + `plots_for_writeup.py:813` superseded this: **qwq-32b does have a ChainScope-computed count (220 → 4.50%) and reproduces ±0** — it is simply excluded from the paper's *headline* (it appears in the figure dict, not the abstract). It is reported as a reproduced-but-non-headline cell. Separately, B4a is distinct from the prior **"B4 cross-validation" Overclaimed row above** (a fabricated 4-model claim) and from the per-trajectory PHR *application* (`validate_b4_arcuschin.py`, GPQA-Diamond, still ⚠) — B4a is the genuine against-release IPHR reproduction.
