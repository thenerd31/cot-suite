...
| 2026-05-29 | B2 Stage B — **Llama-3.1-8B** suggested_answer drop, mean **−36.1pp** (movie −52.5, temporal −52.5, hyperbaton −33.3, causal −37.0, ruin −5.0). ⚠ **CONFOUNDED**: biased-arm malformed 15.2% vs baseline 4.8% (differential dropout) inflates the drop via bias-injection format-degradation. Effective-N per inconsistent cell: movie 29/40, temporal 24/40, hyperbaton 18/27, **causal 27/27 (clean)**, ruin 38/40 | `scripts/validate_b2_turpin_stage_b.py` | `benchmarks/results/llama-3.1-8b/turpin_stage_b/llama-3.1-8b.jsonl` | none — 2026 model, not a Turpin cell | **n/a — not comparable + reduced/confounded pool; NOT a clean bias rate** (only `causal_judgment` −37.0pp at 0 malformed is trustworthy); empty-vs-long-CoT root cause needs a raw-CoT re-query (not persisted) | novel measurement — **CONFOUNDED** (differential dropout) | `c60fde1` |
