"""Modal app hosting Qwen3-8B on a single H100 for Stage 2 scaling (8B variant).

Sibling file to ``scripts/modal_qwen3_14b.py``. Only the following constants
differ between these two files:

    MODEL_ID          = "Qwen/Qwen3-8B"       (vs "Qwen/Qwen3-14B")
    modal.App(...)    = "cotdiv-qwen3-8b"     (vs "cotdiv-qwen3-14b")

Everything else is identical: same vLLM pin (0.19.1), same GPU (H100), same
Modal image, same Volume (shared hf-cache), same Secret (hf-token), same
MAX_MODEL_LEN (40960 — Qwen3-8B has the same 40960 native context as
Qwen3-14B), same class name ``Qwen3Server`` so the driver's Cls.from_name
lookup works uniformly across variants.

# Version pins (locked 2026-04-20, carried forward)
#   vllm==0.19.1        — PyPI latest stable (pypi.org/pypi/vllm/json, 2026-04-18)
#   Qwen/Qwen3-8B       — HF hybrid-mode model (public, same model-card
#                         defaults as 14B per the Qwen3 family spec)
#
# Qwen3-8B recommended inference settings (thinking mode, verbatim from
# HF model card 2026-04-21):
#   temperature=0.6, top_p=0.95, top_k=20, min_p=0
#   max_new_tokens=32768 default; 38912 for complex problems
#   "DO NOT use greedy decoding"
#
# Stage 2 framing: same-family smaller-scale data point for the scaling
...
class Qwen3Server:
    """Single-GPU Qwen3-8B async engine with thinking-mode parsing.
