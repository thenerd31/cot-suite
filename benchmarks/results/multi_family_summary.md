...
tuning), CoT monitorability separates by training paradigm on
**both** the legibility-coverage and PHR axes — though the PHR
separation is smaller than initially reported. **Corrected cluster
separation: 3.5× on the gap axis, 2.76× on the PHR axis, 8/8 models
in their predicted quadrant.** The cross-architecture R1-distill
replication (Llama base + R1 recipe produces the same monitorability
signature class as Qwen base + R1 recipe — Δgap=0.00, ΔPHR=+2.01pp)
confirms that the training intervention, not the model substrate,
controls CoT-answer alignment. The within-family scale comparison
(Qwen2.5-7B vs 72B, ~10× parameters) closes 40% of the legibility-
coverage gap (Δgap=−0.66) while the PHR rate stays elevated and
slightly worsens — the two monitorability axes decouple under scale,
with PHR paradigm-locked and coverage scale-sensitive.
...
the bug. See `AUDIT.md` for the full discovery and fix narrative.
